...
class FluxTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        tokenizer_1_config = (
            ModelConfig(model_id="black-forest-labs/FLUX.1-dev", origin_file_pattern="tokenizer/")
            if tokenizer_1_path is None
            else ModelConfig(tokenizer_1_path)
        )
        tokenizer_2_config = (
            ModelConfig(model_id="black-forest-labs/FLUX.1-dev", origin_file_pattern="tokenizer_2/")
            if tokenizer_2_path is None
            else ModelConfig(tokenizer_2_path)
        )
        self.pipe = FluxImagePipeline.from_pretrained(
            torch_dtype=torch.bfloat16,
            device=device,
            model_configs=model_configs,
            tokenizer_1_config=tokenizer_1_config,
